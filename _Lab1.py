...
class ArbolAVL:
    ...
    def _obtener_nivel_recursivo(self, nodo: Optional[Nodo], nivel_objetivo: int, nivel_actual: int, resultado: List[str]):
        ...
        if nivel_actual == nivel_objetivo:
            resultado.append(f"{nodo.iso3}({nodo.temperatura_media:.3f}°C)")
    ...
    def crear_grafico(self, nombre_archivo: str = "arbol_avl", formato: str = "png", 
        ...
        try:
            ...
            dot.attr(label=f'\\nÁrbol AVL - Temperaturas por País\\n' + 
                          ...
                          f'Rango: {estadisticas.get("temperatura_minima", 0):.3f}°C - {estadisticas.get("temperatura_maxima", 0):.3f}°C',
                    fontsize='14',
                    fontname='Arial Bold')
    ...
    def visualizar_busqueda(self, temperatura_objetivo: float, nombre_archivo: str = "busqueda_avl") -> bool:
        ...
        try:
            ...
            # Agregar título
            if nodo_encontrado:
                titulo = f'Búsqueda exitosa: {temperatura_objetivo}°C\\nEncontrado: {nodo_encontrado.iso3} ({nodo_encontrado.temperatura_media:.3f}°C)'
            else:
    ...
    def _mostrar_arbol_recursivo(self, nodo: Optional[Nodo], prefijo: str, es_ultimo: bool):
        ...
        print(f"{prefijo}{'└── ' if es_ultimo else '├── '}{nodo.iso3} ({nodo.temperatura_media:.3f}°C)")
    ...
    while True:
        ...
        try:
            ...
            if opcion == 1:
                ...
                if padre:
                    print(f"Padre de {nodo.iso3}: {padre.iso3} ({padre.temperatura_media:.3f}°C)")
                else:
                ...
                if abuelo:
                    print(f"Abuelo de {nodo.iso3}: {abuelo.iso3} ({abuelo.temperatura_media:.3f}°C)")
                else:
                ...
                if tio:
                    print(f"Tío de {nodo.iso3}: {tio.iso3} ({tio.temperatura_media:.3f}°C)")
                else:
            ...
            if opcion == 1:  # Insertar nodo manualmente
                ...
                if not arbol.nodos_almacenados:
                    ...
                    for i, nodo in enumerate(paises_ordenados[:15], 1):  # Mostrar primeros 15
                        print(f"   {i}. {nodo.iso3} - {nodo.pais} ({nodo.temperatura_media:.3f}°C)")
                ...
                if ruta_csv:
                    ...
                    if datos_nuevos:
                        ...
                        if confirmar.lower() == 's':
                            ...
                            print(f"   Rango: {stats['temperatura_minima']:.3f}°C - {stats['temperatura_maxima']:.3f}°C")
                            print(f"   Promedio: {stats['temperatura_promedio']:.3f}°C")
